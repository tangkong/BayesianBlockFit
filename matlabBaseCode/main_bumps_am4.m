...
%dir_root =('C:\work\HiTp\MG\Nov2017\J10\Int1D\');
Qmin = 1.5;
Qmax = 5.5;
    ...
    %plotx( id_datum_max_mat( ii_file, 1: num_bumps ), ':r')
     plotx((scalef*(id_datum_max_mat( ii_file, 1: num_bumps ))+startQ),  ':r')
        ...
        for ii_bump = 1: num_bumps -1
           ...
           err_xx_fit = (xx_raw_plot(id_best) - (xx_best + xx_back));
           set(plot((scalef* id_best) + startQ,  (err_xx_fit + 0.5*xx_back), '-k'),'LineWidth',1);
           Err=((err_xx_fit).^2)./(xx_best + xx_back);
         chi_sq(ii_file, ii_bump) = sum(Err)/length(Err);
          MSE(ii_file, ii_bump) = sum((err_xx_fit.^2))/length(err_xx_fit);
          
          % legend('diff','Location','southeast');
%           set(plot((scalef* id_best) + startQ,  (err_xx_fit - min(err_xx_fit)), '-k'),'LineWidth',1); legend('diff');
...
%                 set(plot( id_best,  xx_fit_2, ['-' col_use ] ),'LineWidth',1)
        ...
        for ii_bump = 1: num_bumps -1
           ...
           chi_sq_print = round(100*chi_sq(ii_file, ii_bump), 2);
           chi_sq_list(nj) = chi_sq_print;
           if num_gaussians >1;
             nj = nj + 1;
             chi_sq_list(nj) = chi_sq_print;
           end
           nj = nj +1;
          bumpp = [ num2str( chi_sq_print ), '%'];
           kk=text((scalef*(id_datum_max -10)) +startQ,ii_bump*20,  bumpp );
            set(hh,'FontWeight','bold','FontSize',12)
            if chi_sq_print >=20;
                 set(kk ,'color', 'red', 'FontSize',10)
            else
           set(kk,'FontSize',8)
            end
